import {initialState as defaultMapConfigurationState, MapConfigurationState, reducer} from './map-configuration.reducer';
import {MapConfigurationActions} from '../actions/map-configuration.actions';

describe('MapConfiguration Reducer', () => {
  describe('setMapExtent', () => {
    const initialState: MapConfigurationState = {
      ...defaultMapConfigurationState,
      scaleSettings: {
      ...
      describe('should be true for values around calculatedMaxScale', () => {
        [{value: 100.9}, {value: 100.0}, {value: 99.9}, {value: 98.9}, {value: 0}, {value: -1}].forEach(({value}) => {
          it(`maxZoomedIn is true for scale === ${value}`, () => {
            const actionFloored = MapConfigurationActions.setMapExtent({scale: value, x: 2400000, y: 1200000});
