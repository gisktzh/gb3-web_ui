...
export class EsriDrawingSymbolsService extends BaseApiService implements DrawingSymbolsService {
  public static readonly SYMBOL_COLLECTIONS = {
    'gb3-civil-signatures': {
      label: 'Zivile Signaturen',
      url: 'http://localhost:3000/gb3/v4/gb3-symbols/civil-signatures',
    },
  ...
  public getCollectionInfos() {
    return EsriDrawingSymbolsService.SYMBOL_COLLECTIONS;
  }

  public getCollection(collectionId: keyof typeof EsriDrawingSymbolsService.SYMBOL_COLLECTIONS): Observable<EsriDrawingSymbolChoice[]> {
    if (this.collectionCache.has(collectionId)) {
      return this.collectionCache.get(collectionId)!;
    }

    const collectionUrl = EsriDrawingSymbolsService.SYMBOL_COLLECTIONS[collectionId].url;
