...
import * as hyphenate from 'hyphen/de';
...
describe('HyphenatePipe', () => {
  ...
  describe('Language implementation', () => {
    it('uses the hyphen library in German behind the scenes', () => {
      ...
      expect(hyphenate.hyphenateSync).toHaveBeenCalledWith(value);
    });
  });

  describe('Underscore handling', () => {
    beforeEach(() => {
      vi.spyOn(hyphenate, 'hyphenateSync').mockImplementation((v: string) => v);
    });
